...
export class CategoryService {
  ...
  public async getCategories(query: any) {
    ...
    const { list, total } = await this.repo.getCategories({
      limit,
      offset,
      name,
      shopid,
  ...
  public async createBulkCategory(
    shops: z.infer<typeof createCategorySchema>[],
  ) {
  ...
  public async deleteCategory(id: any) {
    ...
    const hasChildren = await this.repo.hasChildCategories(id);

    if (hasChildren) {
      throw badRequest("Category has sub-categories");
    }

    const inUse = await this.repo.categoryInUse(id);
